...
export class ReceiptChain {
  constructor(secret = typeof process !== "undefined" && process.env.MCP_RECEIPT_SECRET ? process.env.MCP_RECEIPT_SECRET : "local-development-secret") {
    this.secret = secret;
    this.sequence = 0;
    this.previousHash = "GENESIS";
    this._keyPromise = importKey(this.secret);
  }
  ...
  async verify(receipt, previousHash = receipt.sequence === 1 ? "GENESIS" : this.previousHash) {
    ...
    const expectedSignature = await signHmac(key, canonical(unsignedReceipt));
    
    // Constant-time string comparison for the signature
